...
<?php // Display the sidebar on the left side if set.
    ...
    <?php if(have_posts()): ?>
        <?php if(is_search()) : ?>
            <div class="search-results-header">
                <h3><?php printf(__("Search results for: %s", "book_rev_lite"), "<span>" . get_search_query() . "</span>"); ?></h3>
    ...
    <div class="article-container">
        ...
        <?php endwhile; book_rev_lite_pagination(); ?>
    </div><!-- end .article-list -->

<?php else: ?>
    <?php if(is_search()) : ?>
        <div class="search-results-header">
            <h3> <?php printf(__("There are no search results for: %s", "book_rev_lite"), "<span>" . get_search_query() . "</span>"); ?></h3>
        </div><!-- end .search-results-header -->
    <?php endif; ?>
<?php endif; ?>
